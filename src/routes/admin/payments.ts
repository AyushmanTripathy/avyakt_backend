...
router.get("/", async (req, res) => {
  const payments = await Registration.find({ isVerified: false }, "name phoneno upiId");
...
router.get("/all", async (req, res) => {
  try {
    const payments = await Registration.find({}, "name phoneno upiId isVerified isValid")
